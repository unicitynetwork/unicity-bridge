...
per row; nothing is pushed until it says so. Hashes are as of the 2026-09-21
message rewrite (one-line messages, per the repo CLAUDE.md) and, for this
branch, the 2026-09-22 rewrite that removed the analysis document; earlier
hashes in older notes map onto these.
...
|---|---|---|---|---|---|
| `docs/bridging-analysis` | `c2e4fc9` | 2026-09-18 | plan | `docs/dev-plan/09-testnet-e2e.md`: the testnet end-to-end plan (findings, URLs, phases 1 to 3, Docker topology, decisions, risks). The analysis document once committed beside it was removed from the branch on 2026-09-22. | no |
| `feat/sdk3-port` | `dbeacc9` | 2026-09-18 | Phase 1 [2] | `bridge-core` and `bridge-plugin-tron-usdt` moved from `state-transition-sdk` 2.0.0-rc to 3.0.1. Verifier, mint/transfer/verify call sites, demos ported; `demo/sdk3.ts` verification stack; `e2e attach` command for the deployed Nile vault; `DEMO.md`. 48/48 tests, build, typecheck, vectors unchanged, offline attack matrix green. | no |
| `feat/sdk3-port` | `d071774` | 2026-09-18 | Phase 1 [2] | Demo artifact loader pointed at the pre-restructuring path; now resolves `contracts/tron/artifacts/`. Surfaced by the live run. | no |
| `feat/sphere-plugin` (on `feat/sdk3-port`) | `66205f6` | 2026-09-18 | Phase 1 [3], piece 2 | Bridge as a wallet plugin over the sphere-sdk plugin interface. `bridge-core`: structural wallet contract (`WalletTokenPlugin`, `BridgePayments`) + `mintBridgedToken`, `burnForReturn` (persist-then-acknowledge), `recoverPendingBurns`; own test suite. Plugin: value payload in the wallet's format (`SpherePaymentData`, pinned byte-for-byte against sphere-sdk's encoder), `bridgeTokenPlugin()`, `selfMintVerifier()`, `burnIdentifiers()`; demos on the new format; `interop.md` §2.1, `PLUGIN_ARCHITECTURE.md`, README/DEMO updated. 21 files, +566/−90. bridge-core 5/5, plugin 55/55, attack matrix green. | no |
| `feat/sphere-plugin` | `788b209` | 2026-09-21 | Phase 2 step 1 | Prover ported to Rust SDK `v3.0.1` (from the `token-split-2` branch): sdk-ext verifier mirrors the SDK-3 inclusion rules (reference time in the leaf value, deadline check, shard prefix) for anchored mode, v2 transaction/certified-data formats, fixture SMT rebuilt in the new shape. `decode_bridged_payment_data` reads the wallet envelope (tag 39050 v1) and rejects the bare collection; `encode_bridged_payment_data` pinned to the wallet's bytes. Empty burn batches rejected in the guest (§9 hole closed). `BRIDGE_PROTO_VERSION` 2; token vectors regenerated; domain strings and configHash unchanged. 21 files, +368/−134. cargo test 43/43 (4 ignored: 3 old-format live-sample tests, 1 pre-existing), check-vectors ok, SP1 guest binary compiles. vkey not yet computed (no SP1 toolchain / protoc here). | no |
| `feat/sphere-plugin` | `a8c3a74` | 2026-09-21 | Phase 2 step 2 (vkey) + step 3 (container) | `prover/Dockerfile` (guest ELF in Succinct's SP1 v6.3.1 image, host + service with the SP1 host SDK, vkey + check-vectors gate, hardhat artifact, node-slim runtime with relayer), root `docker-compose.yml` (service on :8787, precheck mode, SP1 artifacts volume), permissive CORS on the service router. Built and smoke-tested (/health 200 with CORS, preflight 200). Guest vkey `0x0039a542…`, ELF sha256 `ce8b6ddb…`; local record in the gitignored `sp1-vkey.json`. Not done: `sp1_groth16` mode never exercised in the container (needs the 6 GB artifact download and ~16 GB); relayer still Node. | no |
| `feat/sphere-plugin` | `b84d8eb` | 2026-09-21 | Phase 2 step 2 (deploy) | **v2 vault deployed on Nile**: `TBKJ84417jdxo6j92TxQuYpZdRZGaeZVrv`, tx `6a660031…`, CONFIG_HASH `0xfa77a13a…`, vkey `0x0039a542…`, trust base `0x72a67260…` allow-listed (tx `f22aae9a…`, block 71158544), admin = demo account. `deployments/nile/nile-usdt-v2.json` frozen (config derived by `emit-config`, equal to the chain); `NILE_USDT_BRIDGE` describes v2, `NILE_USDT_BRIDGE_V1` kept for the record; plugin version pin 2 (55/55); entrypoint, compose and run script default to v2; host test checks both freezes against their on-chain hashes. `.env` (gitignored) `TRON_VAULT`/`BRIDGE_VAULT` moved to v2 with v1 kept as a comment. Sphere needs no change: the dev server already serves the rebuilt manifest. Deployer balance 962 TRX before. | no |
| `feat/sphere-plugin` | `e3d25ce` | 2026-09-21 | ops | `docs/OPERATIONS.md`: what runs where, provisioning, keys and what each can do, configuration reference, running the service, where the money-critical state lives, monitoring, failure modes, procedures (validator set change, program change = new vault, admin hand-over, service upgrade), known gaps before real value. | no |
| `feat/sphere-plugin` | `b646b94` | 2026-09-22 | Phase 3 (live run) | Service error classes fixed after the first live burn: a precheck rejection is reported `recoverable: false` (a config mismatch is deterministic; the wallet had looped every 8 s on a v1 token); `POST /returns` re-queues a nullifier whose last attempt failed recoverably (`insert_or_requeue`, `failed_recoverably`), so a wallet retries by posting again. README status semantics, OPERATIONS §8 rows. Service tests 6/6. Image rebuilt and rolled. | no |
| `feat/sphere-plugin` | `e996e23` | 2026-09-22 | Phase 3 (live run) | `BridgePayments.tokenJustification` on the wallet contract; plugin `mintedAgainst(bridge, justification)` decodes the lock justification and compares chain id and lock contract with the bridge's vault, so a wallet can tell v1 tokens from v2 without touching coin id or token type (decision: token identity stays vault-independent). bridge-core 5/5, plugin 56/56. | no |
| `feat/sphere-plugin` | `42d8648` | 2026-09-22 | Phase 4 (batching) | `BRIDGE_BATCHING.md`: the brief for a fresh-context agent to plan and build in-circuit batching in the service. Records that the guest, the host (`build_certified_guest_input_batch`) and the vault already take batches and the service does not; constraints, required properties, restart matrix, tests, open decisions. | no |
...
| `feat/bridge-v2` (from `origin/main` `762d350d`) | `8b308813` | 2026-09-21 | Phase 1 [4] | The bridge as a **wallet module**, not a cherry-pick: `src/modules/registry.ts` globs `src/modules/<name>/module.ts`; the core knows the contract only (token plugins for `Sphere.init`, `describeCoin` for rows, actions with their own screen). `src/modules/bridge/` is assets-in only and is itself pluggable: `assets/<name>/index.ts` per bridgeable asset (`tron-usdt` today), the screen and `bridgeIn.ts` name no chain; mint via bridge-core `mintBridgedToken`. Core touch points: `SphereProvider` (plugins), `L3WalletView` (`ModuleActions`, `describeCoin`), `AssetRow` badge, `vite.config` dedupe of state-transition-sdk. `package.json`: sphere-sdk + both bridge packages as `file:` links (tripwire in `dependency-hygiene.test.ts`); `tronweb` for the dev-key signer. Docs: `docs/WALLET-MODULES.md`. 29 files, +2347/−214. Gate: `tsc -b`, `typecheck:tests`, eslint 0 errors, `vite build`, vitest 1531/1531 (1 skipped tripwire), `npm run dev` serves the module graph with every linked import resolving. The 18 old UI commits stay unused. | no |
| `feat/bridge-v2` | `39b38a03` | 2026-09-21 | Phase 1 [4] | Module screens mounted with the built-in modals (the tab bar bled through); signer availability re-checked while the screen is open (TronLink injects late). | no |
| `feat/bridge-v2` | `e4e252d5` | 2026-09-21 | Phase 1 [4] | Picker: network → asset → form, every step shown even with one option; `BridgeAsset.chain` descriptor (id, family, network name, testnet flag); component test walks the steps. | no |
| `feat/bridge-v2` | `35b34e32` | 2026-09-21 | Phase 1 [5] | A module's coin shown as the module says everywhere (`moduleAssetView` / `moduleTokenView` in `useAssets` / `useTokens`), reference price for USDT; long names expand on click; send dialog header wraps. Fixes "Available 10000000 F16348", the $0 column and the 64-hex header. | no |
| `feat/bridge-v2` | `4c946dfa` | 2026-09-21 | Phase 1 [5] | Approval is exactly the deposit amount; max-approve option removed. | no |
| `feat/bridge-v2` | `ffd27a43` | 2026-09-21 | Phase 3 (UI) | **Assets out.** Direction step in front of the picker; bridge-out form (tokens of the coin, each burned whole, destination validated by the asset); returns list on the first screen with the service's status and the settle tx. `bridgeOut.ts`: burn → record the blob → release the wallet copy → submit; `syncReturns` resubmits unacknowledged or forgotten returns; `recoverBurns` on wallet start. `BridgeAsset.out` (reasonFor / identify / returns) filled by the Tron asset from the plugin's bridge-back surface; service URL from the manifest or `VITE_BRIDGE_RETURN_SERVICE_URL`. 9 new tests. | no |
| `feat/bridge-v2` | `bc94dc41` | 2026-09-22 | Phase 3 (live run) | After the first live burn (a v1 token, refused by the v2 service): the picker offers only tokens the active vault backs (`useReturnableTokens` → `splitReturnable` → `payments.tokenJustification` + `out.backs`), others listed as not returnable here; a `failed` return the service calls recoverable stays active, is resubmitted after 60 s or from a retry button, final failures are dismissable; returns row address expands on click; post-burn screen reads the record's live status (`returnStatusSentence`) instead of claiming "proving". 11 files. Bridge module 39/39, full suite green apart from two wallet-lock files that pass alone. | no |
...
| Branch | Commit | Date | Plan step | Summary | Pushed |
|---|---|---|---|---|---|
| `feat/token-plugins` | `5beaca67` | 2026-09-18 | Phase 1 [3], piece 1 | Generic token-plugin interface, no bridge code: `TokenPlugin` (mint-reason verifiers by tag) registered via `EngineConfig.plugins` / `SphereInitOptions.plugins`; `mintDataToken` gains `justification` + per-mint verifiers; `ITokenEngine.burn` (BurnPredicate(sha256(reason)), deterministic realization); payments-v2 `mintCustom`, `burn`, `pendingBurns`, `acknowledgeBurn`, both journal-first with crash replay. 24 files, +1156/−58. typecheck, typecheck:tests, lint 0 errors, build, vitest 140 files / 2732 tests. | no |
| `feat/token-plugins` | `9f3a7ee6` | 2026-09-22 | Phase 3 (live run) | `ITokenEngine.readTokenJustification` and payments-v2 `tokenJustification(tokenId)` next to `tokenData`: the genesis mint reason of a held token, for plugins that must tell which backing a token has. Fake engine carries the justification. 10 files. vitest 140 files / 2735 tests. | no |
