...
| `feat/sphere-plugin` (on `feat/sdk3-port`) | `66205f6` | 2026-09-18 | Phase 1 [3], piece 2 | Bridge as a wallet plugin over the sphere-sdk seams. `bridge-core`: structural wallet contract (`WalletTokenPlugin`, `BridgePayments`) + `mintBridgedToken`, `burnForReturn` (persist-then-acknowledge), `recoverPendingBurns`; own test suite. Plugin: value payload in the wallet's format (`SpherePaymentData`, pinned byte-for-byte against sphere-sdk's encoder), `bridgeTokenPlugin()`, `selfMintVerifier()`, `burnIdentifiers()`; demos on the new format; `interop.md` §2.1, `PLUGIN_ARCHITECTURE.md`, README/DEMO updated. 21 files, +566/−90. bridge-core 5/5, plugin 55/55, attack matrix green. | no |
