...
A versioned fixture package `protocol/vectors/` is the
...
|---|---|---|
| `config`, `lock`, `reason`, `nullifier`, `public` | **Rust** (`protocol/vectors/gen`) | the circuit is the strictest consumer; generate where the relation lives |
...
- Each consumer pins `protocol/vectors` by `BRIDGE_PROTO_VERSION` and fails on mismatch.
...
- This document + `protocol/vectors` move together; a change to either bumps
