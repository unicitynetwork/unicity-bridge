 ...
 * Rust-generated `bridge-vectors` byte-for-byte (00 §10; TS must pass config,
...
function vector(group: string, file: string): any {
  const url = new URL(`../../bridge-vectors/${group}/${file}`, import.meta.url);
...
test('VERSION matches the pinned vectors', () => {
  const v = readFileSync(new URL('../../bridge-vectors/VERSION', import.meta.url), 'utf8').trim();
  assert.equal(v, '1');
});
