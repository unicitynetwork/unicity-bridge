...
//!     ../packages/bridge-plugin/demo/.bridge-back-state.json ../bft-trustbase.testnet2.json
use std::{env, fs};
...
fn main() {
    let blob = env::args()
        .nth(1)
        .unwrap_or_else(|| "../packages/bridge-plugin/demo/.bridge-back-state.json".to_string());
    let tb = env::args()
        .nth(2)
