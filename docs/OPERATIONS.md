...
| Settlement fails with `OUT_OF_TIME: CPU timeout` and burns millions of energy | the node build executes the Groth16 verifier past Tron's 80 ms per-transaction CPU limit (seen on Nile 4.8.2.2.PQ1 on 2026-09-22, when even the repository's previously verified bundle stopped verifying) | nothing on the service side; stop the service so retries do not each pay the full fee, and check with a free `triggerconstantcontract` of `verifyProof` on the verifier before starting it again; a retry reuses the proof |
