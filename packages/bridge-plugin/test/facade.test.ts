 ...
 * (`deployments/nile/nile-usdt.json`, cross-stack freeze).
 */
...
function frozen(): any {
  const url = new URL('../../../deployments/nile/nile-usdt.json', import.meta.url);
...
test('buildBridgeInPlan derives tokenId + recipientCommitment + the unsigned calls', async () => {
  const [loaded] = loadBridges(NILE_USDT_BRIDGE, deps());
  ...
  assert.equal(plan.approve.contractHex, loaded.plugin.resolvedConfig.assetContractHex);
  assert.equal(plan.approve.parameters[0].value, loaded.plugin.resolvedConfig.lockContractHex);
  ...
  assert.equal(plan.lock.contractHex, loaded.plugin.resolvedConfig.lockContractHex);
