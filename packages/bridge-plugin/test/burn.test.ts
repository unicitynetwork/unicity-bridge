 ...
 * cross-checked against `protocol/vectors`; the SDK binding is checked by decoding
 * the predicate back.
 */
...
function vector(group: string, file: string): any {
  return JSON.parse(readFileSync(new URL(`../../../protocol/vectors/${group}/${file}`, import.meta.url), 'utf8'));
}
