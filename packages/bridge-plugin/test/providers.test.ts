 ...
 * `create`s a `SourceSigner` the bridge-in flow drives uniformly.
 */
import assert from 'node:assert/strict';
import { test } from 'node:test';

import { availableTronWallets, type SourceSigner } from '../src/wallet/index.js';

const NILE = 3448148188;
const fakeSigner = (): SourceSigner => ({
  connect: async () => '',
  getAddress: async () => '',
...
test('a provider.create builds a SourceSigner for the given chainId', () => {
