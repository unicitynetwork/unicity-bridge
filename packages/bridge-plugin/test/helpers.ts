...
export const CONFIG: BridgeAssetConfig = {
  family: 'tron',
...
export class MockTronRpc implements SourceChainRpc {
  public constructor(
    public txInfo: SourceTxInfo | null,
    public tip: bigint,
  ) {}

  public async getTransactionInfo(): Promise<SourceTxInfo | null> {
    return this.txInfo;
  }
...
export function makeLockLog(addressEvmHex: string, e: LockEventFields): SourceLog {
  return {
...
export interface Scenario {
  plugin: ReturnType<typeof createBridgePlugin>;
  rpc: MockTronRpc;
...
export async function buildScenario(
  overrides: {
    ...
    tip?: bigint;
    success?: boolean;
    logs?: (defaultLog: SourceLog) => SourceLog[];
  ...
  const plugin = createBridgePlugin(CONFIG, { rpc: new MockTronRpc(null, 0n) });
  ...
  const wired = createBridgePlugin(CONFIG, { rpc });

  let jData: BridgeLockJustification['data'] = {
  ...
  const justification = new BridgeLockJustification(jData);
