...
test('justification round-trips through CBOR', () => {
  const original = new BridgeLockJustification({
    chainId: 3448148188,
  ...
  const decoded = BridgeLockJustification.fromCBOR(original.toCBOR());
...
test('decoding rejects a foreign CBOR tag', () => {
  ...
  assert.throws(() => BridgeLockJustification.fromCBOR(bogus), /Invalid CBOR tag/);
});

test('the tag is the allocated bridge tag', () => {
  assert.equal(BridgeLockJustification.CBOR_TAG, BRIDGE_LOCK_JUSTIFICATION_TAG);
  assert.equal(BRIDGE_LOCK_JUSTIFICATION_TAG, 1330002n);
});
