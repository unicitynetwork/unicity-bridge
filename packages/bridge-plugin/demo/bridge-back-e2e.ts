 ...
 *               `BridgeLockJustification` (tag 1330002), structurally verified
...
class MockTronRpc implements SourceChainRpc {
  public constructor(
    private readonly info: SourceTxInfo,
...
async function main(): Promise<void> {
  ...
  const lockContractHex = createBridgePlugin(pluginConfig, {
  ...
  const plugin = createBridgePlugin(pluginConfig, {
  ...
  const justification = new BridgeLockJustification({
    ...
    JSON.stringify(
      {
        ...
        proverConfig: {
          ...
          justificationTag: BridgeLockJustification.CBOR_TAG.toString(),
        },
        lock: {
